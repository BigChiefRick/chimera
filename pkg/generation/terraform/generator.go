...
func (g *Generator) GenerateModule(config generation.ModuleConfig) (map[string]string, error) {
	...
	// Add module resources if provided
	if len(config.Resources) > 0 {
		// Sort resources by dependency order
		sortedResources := g.sortResourcesByDependencies(config.Resources)
	...
	// Collect all variables from resources
	variables := make(map[string]generation.Variable)
	...
	// Collect all outputs from resources
	outputs := make(map[string]generation.Output)
	for _, resource := range config.Resources {
		for name, output := range resource.Outputs {
			// Prefix with resource name to avoid conflicts
	...
	// Generate versions.tf if providers are specified
	if len(config.RequiredProviders) > 0 {
		...
		for name, version := range config.RequiredProviders {
			versionsContent.WriteString(fmt.Sprintf("    %s = {\n", name))
			versionsContent.WriteString(fmt.Sprintf("      source  = \"hashicorp/%s\"\n", name))
